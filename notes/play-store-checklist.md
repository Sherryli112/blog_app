...
### 第三版（0.0.3）— 圖片主機修正 + 分類頁空清單修正

- [x] 文章內文圖片主機從 `mgmt.funtime.com.tw` 改寫成對外的 `upd-api.funtime.com.tw`（iOS 版實機測試證實 mgmt 對外部網路 403）✅ 2026-07-17
- [x] 分類頁在 regions 回傳空陣列時補上提示畫面，不再顯示空白 ✅ 2026-07-17
- [x] versionCode 4 / versionName 0.0.3，`bundleRelease` 建置簽署成功 ✅ 2026-07-17
- [ ] commit 推上 `origin/pre-gamification`
- [ ] Play Console →「FunTime部落格」→ 內部測試 → 建立新版本，上傳這份 AAB
- [ ] 填寫版本資訊（release 標籤可填「0.0.3」）
- [ ] 發布後**務必用真機關掉公司 WiFi、走行動網路**測試文章內文圖片是否正常顯示（這個問題只有外部網路才會現形，公司網路/模擬器測不出來）
